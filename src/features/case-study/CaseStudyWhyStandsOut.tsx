...
const CaseStudyWhyStandsOut = ({ cms = {} }: Props) => {
  const checkItems = DEFAULT_CHECK_ITEMS.map((item, i) => cms[`check${i + 1}`] || item);

  return (
    <div
      className="relative bg-[#0a1628] w-full max-w-[1300px] mx-auto rounded-2xl py-12 md:py-16 lg:py-24 overflow-hidden bg-cover bg-center bg-no-repeat mt-12 lg:mt-24 lg:h-[700px] h-auto"
      style={{ backgroundImage: cms.bg_image ? `url(${cms.bg_image}), url('/whyTStandBg.png')` : `url('/whyTStandBg.png')` }}
    >
      ...
      <div className="px-4 relative z-10">
        <div className="flex flex-col md:flex-row items-center gap-10 md:gap-20 lg:gap-28">
          ...
          <div className="w-full md:w-1/2 relative">
            ...
            >
              <Image
                src={cms.image1 || whyStand1}
                alt="Why TrackForce Stands Out"
                width={1200}
                height={600}
                quality={90}
                className="lg:w-[470px] w-full max-w-[320px] lg:max-w-[470px] object-contain"
                onError={(e) => { e.currentTarget.src = whyStand1.src; }}
              />
            </motion.div>
            <motion.div
          ...
          <div className="w-full md:w-1/2 mt-8 md:mt-0 text-center md:text-left">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-6 md:mb-8">
              {cms.heading || <>Why TrackForce{" "}<span className="font-playball font-normal">Stands Out</span></>}
            </h2>
