...
type WhyChooseUsProps = { cms?: Record<string, string> };
  ...
  const features = [
    {
      icon: <AutomicIcon />,
      color: '#F3A7D2',
      title: cms.point1_title || 'Automatic Time & Activity Tracking',
      desc: cms.point1_desc || "Work hours, app usage, and active time are recorded automatically, so there's no need for spreadsheets or daily logs.",
    },
    {
      icon: <CustomMonitoringIcon />,
      color: '#BDFCB5',
      title: cms.point2_title || 'Custom Monitoring Controls',
      desc: cms.point2_desc || "Decide what to track, when to track, and for whom – fully adjustable to match your team policies and privacy standards.",
    },
    {
      icon: <SecureIcon />,
      color: '#B0CDFF',
      title: cms.point3_title || 'Secure & Built to Scale',
      desc: cms.point3_desc || "Enterprise-grade security with infrastructure that supports growing teams – from small groups to large organizations.",
    },
    {
      ...
      color: '#FCBD7A',
      title: cms.point4_title || 'All-in-One Workforce Monitoring',
      desc: cms.point4_desc || "Track employee activity, productivity, and work patterns from a single dashboard – no switching tools, no fragmented data.",
    },
    {
      icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      color: '#C8B5FC',
      title: cms.point5_title || 'Real-Time Work Visibility',
      desc: cms.point5_desc || "See what your team is working on during work hours, as it happens – without constant check-ins or manual updates.",
    },
    {
      icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      color: '#83E7FD',
      title: cms.point6_title || 'Behavior-Based Productivity Insights',
      desc: cms.point6_desc || "Identify idle time, focus spots, and performance patterns using real activity data – not assumptions or self-reports.",
    },
  ];
  return (

    <section
      className=" w-full py-16 sm:py-20 lg:py-24 text-white bg-cover bg-center bg-no-repeat bg-[url('/why-choose-us-bg.png')]"
      style={cms.bg_image ? { backgroundImage: `url(${cms.bg_image}), url('/why-choose-us-bg.png')` } : {}}
    >
      <div className='max-w-[1300] mx-auto px-4 sm:px-6 xl:px-0'>
        <div className="flex flex-col lg:flex-row gap-8 w-full px-4 sm:px-6 lg:px-4">
          ...
          <div className="w-full lg:w-2/5 flex flex-col justify-center items-center lg:items-start text-center lg:text-left px-2 md:px-8 order-1 lg:order-2">
            ...
            <RichText
              className="text-white text-sm sm:text-base mb-6 max-w-lg mt-6"
              html={cms.description || "Track employee activity, productivity, and work patterns in one place without disrupting everyday workflows."}
            />
            <div className="">
              <Link href={cms.cta_url || "/contact"} className="group custom-button relative w-fit bg-white text-black font-bold py-3 px-6 rounded-[16px] flex gap-3 justify-between items-center mx-auto lg:mx-0">
                <span className='group-hover:italic '>{cms.cta_text || "Book a Demo"}</span> <FaArrowRightLong className='group-hover:-rotate-[30deg] transition ease-in-out' />
                <motion.div
