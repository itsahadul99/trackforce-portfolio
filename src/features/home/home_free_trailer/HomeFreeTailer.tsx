...
type HomeFreeTailerProps = { cms?: Record<string, string> }

const HomeFreeTailer = ({ cms = {} }: HomeFreeTailerProps) => {
    const sectionRef = useRef<HTMLDivElement>(null);
    const inView = useInView(sectionRef, { once: true, margin: "0px 0px 400px 0px" });

    const ctaUrl = cms.cta_url || "https://app.trackforce.io/3/Professional/signup?trial=true";
    const ctaSecondaryUrl = cms.cta_secondary_url || "/contact";
        ...
        <div ref={sectionRef} className="group/free_trailer relative overflow-hidden max-w-[1300] mx-4 sm:mx-6 lg:mx-auto min-h-[480px] lg:h-[450px] bg-[#10243E] text-white flex flex-col lg:flex-row gap-8 lg:gap-20 items-center lg:justify-between pl-6 sm:pl-8 lg:pl-10 pr-6 sm:pr-8 lg:pr-0 py-10 lg:py-0 rounded-2xl px-4">
            ...
            <div className="w-full lg:w-1/2 relative z-10 text-center lg:text-left">
                ...
                <div className="mt-8 lg:mt-12 flex flex-wrap justify-center lg:justify-start gap-4 lg:gap-5">
                    ...
                    >
                        <span className='group-hover:italic'>{cms.cta_text || "Start Free Trial"}</span>
                    ...
                    <Link href={ctaSecondaryUrl} className='py-3 px-6 rounded-[16px] border cursor-pointer'>
                        {cms.cta_secondary || "Book a Demo"}
                    </Link>
                </div>
            </div>
            <div className="hidden lg:block w-1/2 relative z-50! h-[450px]">
                <Image
                    src={cms.dashboard_image || dashboard}
                    alt="Dashboard"
                    width={1200}
                    height={800}
                    quality={90}
                    className="absolute -bottom-36 -right-6 scale-110 transition-all duration-600 ease-in-out group-hover/free_trailer:-translate-y-36"
                />
            </div>
        </div>
    )
