import { getHowSections } from '@/lib/cms';
...
const HowTrackforceWorks = async ({ cms = {} }: HowTrackforceWorksProps) => {
  const cmsSections = await getHowSections();
    ...
    <section className="max-w-[1300] mx-4 sm:mx-6 xl:mx-auto px-5 sm:px-10 lg:px-[93px] py-12 sm:py-16 lg:py-[96px] -mt-[80px] sm:-mt-[150px] lg:-mt-[250px] rounded-4xl bg-cover bg-center bg-no-repeat bg-[url('/trackforce_work_bg.png')]">
      <div className="container mx-auto text-center relative z-10">
        ...
        <h2 className="text-2xl sm:text-3xl lg:text-[42px] font-semibold mb-6 text-white leading-tight">
          Built for Managers <span className="font-playball font-normal">Who Need Clarity,</span>{' '}
          <br className="hidden sm:block" /> Not Micromanagement
        </h2>
      ...
      {cmsSections.length > 0 ? (
        cmsSections.map((section, idx) => (
          <HowSection key={section.id} section={section} reversed={idx % 2 !== 0} />
        ))
      ) : (
        <>
